...
it('deletes all specialist hours', async () => {
  const user = {
  ...
  expect(await validTokenCaller.deleteAllSpecialistHours()).toMatchObject({})
  expect(await validTokenCaller.deleteAllSpecialistHours()).toEqual(undefined)
})
...
it('should throw an error for unauthenticated change', async () => {
  ...
  await expect(
    unauthenticatedCaller.deleteAllSpecialistHours()
